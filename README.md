...
- `:Persisted clean` - Clean up invalid sessions where the working directory no longer exists
...
**Shortening long session paths in Telescope**

By default, long paths are shortened by replacing the home directory with
`~`. If you need more control (e.g. shortening a Windows user path under
WSL), use `telescope.path_aliases` to declare prefix substitutions:

```lua
require("persisted").setup({
  telescope = {
    path_aliases = {
      { from = "/mnt/c/Users/gaoqiang", to = "/mnt/c/~" },
    },
  },
})
```

The longest matching `from` prefix wins, so you can declare multiple
overlapping aliases. For full control over the displayed string (e.g. to
apply Lua pattern-based substitutions or any custom logic), set
`telescope.display` instead --- it takes precedence over `path_aliases`:

```lua
require("persisted").setup({
  telescope = {
    display = function(path)
      -- `path` is the session's absolute directory path
      return path:gsub("^/mnt/c/Users/gaoqiang", "/mnt/c/~")
    end,
  },
})
```
